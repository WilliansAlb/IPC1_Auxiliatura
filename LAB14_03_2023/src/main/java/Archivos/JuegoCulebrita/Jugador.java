...
public class Jugador implements Serializable {
    ...
    public void ingresarPosicionNueva() {
        if (getPosiciones() != null) {
            Posicion[] nuevasPosiciones = new Posicion[getPosiciones().length + 1];
            for (int i = 0; i < getPosiciones().length; i++) {
                nuevasPosiciones[i] = getPosiciones()[i];
            }
            nuevasPosiciones[getPosiciones().length] = new Posicion(-1, -1);
            setPosiciones(nuevasPosiciones);
        } else {
            Posicion[] nuevasPosiciones = {new Posicion(-1, -1)};
            setPosiciones(nuevasPosiciones);
        }
    }
}
